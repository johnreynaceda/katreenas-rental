<div>
      ...
      <section class="content">
        <div class="container-fluid">
            <div class="row mb-3">
                @if ($updateMode)
                @include('livewire.boarder.boarder-update')
                @else
                @include('livewire.boarder.boarder-create')
                @endif
            </div>
            <div class="row">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                          <h3 class="card-title">List of Boarders</h3>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                            <div class="col-lg-6">
                                <div class="input-group mb-3">
                                    ...
                                    <input wire:model="search" type="text" class="form-control" placeholder="Search Boarder">
                                  </div>
                            </div>
                          <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tr>
                                <td>{{ $i++ }}</td>
                                <td class="uppercase">{{ $boarder->lastname.", ".$boarder->firstname." ".$boarder->middlename[0]."." }} </td>
                                <td>{{ $boarder->address }}</td>
                                <td>{{$boarder->contact}}</td>
                                <td>{{ $boarder->room->room }}</td>
                                <td>
                                    @foreach($boarder->appliances as $appliance)
                                    <ul>
                                        <li>{{ $appliance->appliance }}({{ $appliance->payment }})</li>
                                    </ul>
                                 @endforeach
                                </td>
                                <td>
                                    <div class="row space-x-1"  >
                                       <div class="edit" x-data="{ show: @entangle('updateModal') }">
                                        <button wire:click.prevent="edit({{ $boarder->id }})" class="btn btn-warning"><i class="fa fa-edit"></i></button>
                                        <div tabindex="0" class="z-40 overflow-auto left-0 top-0 bottom-0 right-0 w-full h-full fixed" style="background-color: rgba(0,0,0,.5);" x-show="show">
                                            <div class="text-left bg-white h-auto mx-72  mt-20" @click.away="ss = false">
                                                <div class="flex bg-nav text-side">
                                                    <div class=" px-3 py-2 font-medium text-lg w-11/12">
                                                     <h4>Update Boarder</h4>
                                                 </div>
                                                    ...
                                                    <form class="flex space-x-7 py-2 px-4" action="">
                                                        ...
                                                        <div class=" w-6/12">
                                                            ...
                                                            <div class="mb-2 flex justify-end space-x-2">
                                                               ...
                                                               <button wire:click.prevent="saveedit" class="bg-green-600 focus:outline-none flex rounded-md shadow-md text-white hover:bg-green-600 cursor-pointer p-2">
                                                                 
                                                                   <h5 class="">SAVE</h5>
                                                               </button>
                                                               <button wire:click.prevent="canceledit" class="bg-red-600 focus:outline-none flex rounded-md shadow-md text-white hover:bg-red-600 cursor-pointer p-2">
                                        ...
                                        <button wire:click.prevent="delete({{ $boarder->id }})" class="btn btn-danger"><i class="fa fa-trash"></i></button>
                                        <div class="qr" x-data="{ show: @entangle('qrmodal') }">
                                            <button wire:click.prevent="qr({{ $boarder->id }})" wire:key="qr" class="btn btn-success"><i class="fa fa-qrcode"></i></button>
                                        <div tabindex="0" class="z-40 overflow-auto left-0 top-0 bottom-0 right-0 w-full h-full fixed" style="background-color: rgba(0,0,0,.5);" x-show="show">
                                            <div class="text-left bg-white h-auto mx-96  mt-20" @click.away="show = false">
                                                ...
                                                <div class="body bg-white text-side ">
                                                    <form class="flex space-x-7 justify-center py-2 px-4" action="">
                                                        <div class=" w-12/12">
                                                           <div class="qr" id="qr">
                                                            {!! QrCode::size(200)->generate($this->qr.'boarder') !!}
                                                           </div>
                                                            <div onclick="printJS('qr', 'html')" class="bg-green-600 text-white hover:bg-green-800 cursor-pointer mt-3 text-center">
                                                                <span>PRINT</span>
                                                            </div>
                                                        </div>
                                                    </form>
